...
select 
	id, first_name, last_name, age, null as phone, 'Bremen' as city
from user_bremen
union all
select 
	id, first_name, last_name, age, phone, 'Berlin' as city
from user_berlin;
...
-- select 
-- 	t1.title, t1.description, t2.name
-- from film t1
-- inner join language t2
-- on t1.language_id = t2.language_id;

select 
	t1.first_name, t1.last_name, t3.title
from actor t1
...
-- 3. Вывести имя, фамилию, зарплату, должность, 
-- минимальную и максимальную зарплату для этой должности только тех сотрудников, у которых зарплата равняется либо минимальной зарплате, либо максимальной.
select
